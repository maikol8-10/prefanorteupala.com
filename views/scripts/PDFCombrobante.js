function jsPDFInvoiceTemplate(props) {
    ...
    var tdWidth = (doc.internal.pageSize.getWidth() - 20) / param.invoice.header.length;
    ...
    if (param.invoice.header.length > 2) { //add style for 2 or more columns
        ...
        tdWidth = (doc.internal.pageSize.getWidth() - 20 - customWidthOfAllColumns) / (param.invoice.header.length - customColumnNo.length);
    }
    //#endregion
...
let props = {
    ...
    compress: true,
    logo: {
        src: "../public/images/prefaNorte.png",
    ...
    invoice: {
        ...
        invGenDate: "Fecha: 02/02/2021 10:17",
        headerBorder: false,
...
const generarPDF = async (idVenta) => {
    ...
    let venta = {}, detalleVenta = [];
    await $.post("../ajax/venta.php?op=getVenta", {idVenta: idVenta}, function (data, status) {
        venta = JSON.parse(data);
    });

    await $.post("../ajax/venta.php?op=getDetalleVenta", {idVenta: idVenta}, function (data, status) {
    ...
    let props = {
        ...
        business: {
            //name: "Miguel Mora Alvarez",
            email_1: "Ced. 204040153",
            address: "100 Oeste del cementerio de San Fernando de Upala ",
            phone: "24701818-63869238-88127359",
            email: "prefanortefactura@gmail.com ",
            website: "Miguel Mora Alvarez",
        },
        contact: {
            label: "Cliente:",
            name: venta.cliente,
            //address: "Albania, Tirane, Astir",
            phone: "Tipo Pago: " + venta.tipoPago,
            //email: "client@website.al",
            //otherInfo: "www.website.al",
        },
        invoice: {
            label: "Factura #: ",
            num: venta.numeroComprobante,
            //invDate: "Payment Date: 01/01/2021 18:12",
            invGenDate: "Fecha: " + venta.fecha,
            headerBorder: false,
            tableBodyBorder: false,
            header: [
                {
            ...
            table: detalleVenta.data.map((item, index) => [
                index + 1,
                item.categoria,
                item.descripcion,
                item.precioVenta,
            ...
            additionalRows: [
                ...
                {
                    col1: 'IVA:',
                    col2: myRound(venta.iva, 2).toString(),
                    col3: '',
                    style: {
                        fontSize: 10 //optional, default 12
                    }
